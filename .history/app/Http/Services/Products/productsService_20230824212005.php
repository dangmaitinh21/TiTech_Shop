...
class productsService {
    ...
    protected function isValidPrice($request) {
        if((float) $request->input('productPrice') !== 0  && (float) $request->input('productPriceSale') && (float) $request->input('productPrice' <= (float) $request->input('productPriceSale')) ) {
    ...
    public function updateProduct($product, $request) {
        if(!($this->isValidPrice($request))) return false;
        dd((float) $request->input('productPrice'));
        // try {
