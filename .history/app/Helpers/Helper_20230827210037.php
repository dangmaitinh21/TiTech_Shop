...
class Helper {
    ...
    public static function listCategories($categories, $parent_id=0, $char='', $perPage = 15, $currentPage = null) {
        $filteredCategories = [];
        foreach ($categories as $key => $category) {
            if($category->parent_id === $parent_id) {
        ...
        $currentPage = $currentPage ?: Paginator::resolveCurrentPage('page');
    
        $categoryCollection = new Collection($filteredCategories);
    
        $paginator = new LengthAwarePaginator(
            $categoryCollection->forPage($currentPage, $perPage),
            count($categoryCollection),
            $perPage,
            $currentPage,
